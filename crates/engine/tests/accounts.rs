...
use rustcash_engine::{account::AccountService, EngineError};
...
async fn create_inserts_account(pool: SqlitePool) {
    ...
    AccountService::new(pool.clone()).create(&acct).await.unwrap();

    let found = AccountRepository::new(pool).find_by_id(acct.id).await.unwrap().unwrap();
...
async fn rename_updates_name_and_full_name(pool: SqlitePool) {
    ...
    AccountService::new(pool.clone())
        .rename(acct.id, "Current Assets".to_string(), "Current Assets".to_string())
        .await.unwrap();
...
async fn rename_cascades_full_name_to_children(pool: SqlitePool) {
    ...
    let mut grandchild = insert_account(&pool, book.id, commodity.id, "Petty", AccountType::Cash).await;
    grandchild.parent_id = Some(child.id);
    grandchild.full_name = "Assets:Cash:Petty".to_string();
    repo.update(&grandchild).await.unwrap();

    AccountService::new(pool.clone())
        .rename(root.id, "Current Assets".to_string(), "Current Assets".to_string())
        .await.unwrap();
...
async fn soft_delete_removes_from_active_accounts(pool: SqlitePool) {
    ...
    AccountService::new(pool.clone()).soft_delete(acct.id).await.unwrap();

    let active = AccountRepository::new(pool).find_by_book(book.id).await.unwrap();
    assert!(active.is_empty());
}

#[sqlx::test(migrations = "../storage/migrations")]
async fn soft_delete_unknown_account_returns_error(pool: SqlitePool) {
    let err = AccountService::new(pool).soft_delete(AccountId::new()).await.unwrap_err();
