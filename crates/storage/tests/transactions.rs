...
async fn find_by_id_returns_none_for_missing(pool: SqlitePool) {
    let repo = TransactionRepository::new(pool);
    assert!(
        repo.find_by_id(TransactionId::new())
            .await
            .unwrap()
            .is_none()
    );
...
async fn find_by_account_returns_multiple_transactions_ordered_by_date(pool: SqlitePool) {
    ...
    let mut txn_later = make_transaction(
        book.id,
        checking.id,
        expenses.id,
        commodity.id,
    ...
    let mut txn_earlier = make_transaction(
        book.id,
        checking.id,
        expenses.id,
        commodity.id,
