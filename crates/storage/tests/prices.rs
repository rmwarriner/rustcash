...
async fn find_by_book_returns_prices_ordered_by_date_desc(pool: SqlitePool) {
    ...
    for date in dates {
        repo.insert(&make_price(book.id, aapl.id, usd.id, date, Decimal::new(100, 0)))
            .await.unwrap();
    }
...
async fn find_by_book_excludes_other_books(pool: SqlitePool) {
    ...
    repo.insert(&make_price(book_a.id, c_a.id, usd_a.id, date, Decimal::new(100, 0))).await.unwrap();
    repo.insert(&make_price(book_b.id, c_b.id, usd_b.id, date, Decimal::new(200, 0))).await.unwrap();
...
async fn latest_before_returns_most_recent_on_or_before(pool: SqlitePool) {
    ...
    let prices = [
        (NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),  Decimal::new(18000, 2)),
        (NaiveDate::from_ymd_opt(2024, 1, 15).unwrap(), Decimal::new(18500, 2)),
        (NaiveDate::from_ymd_opt(2024, 2, 1).unwrap(),  Decimal::new(19000, 2)),
    ];
    for (date, value) in prices {
        repo.insert(&make_price(book.id, aapl.id, usd.id, date, value)).await.unwrap();
    }

    // Jan 20 → latest on or before is Jan 15
    let found = repo
        .latest_before(aapl.id, usd.id, NaiveDate::from_ymd_opt(2024, 1, 20).unwrap())
        .await.unwrap().unwrap();
...
async fn latest_before_returns_none_when_no_price_exists(pool: SqlitePool) {
    ...
    let found = repo
        .latest_before(aapl.id, usd.id, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap())
        .await.unwrap();
...
async fn latest_before_matches_exact_date(pool: SqlitePool) {
    ...
    repo.insert(&make_price(book.id, aapl.id, usd.id, date, Decimal::new(18500, 2)))
        .await.unwrap();

    let found = repo.latest_before(aapl.id, usd.id, date).await.unwrap().unwrap();
...
async fn update_changes_date_value_and_source(pool: SqlitePool) {
    ...
    repo.update(price.id, new_date, new_value, PriceSource::Import, Utc::now())
        .await.unwrap();
